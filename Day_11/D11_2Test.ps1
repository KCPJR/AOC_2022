...
Function D11_2test{
    ...
    if(!$true){
    write-host ("After round {0}, the monkeys are holding items with these worry levels:" -f $round)
        $mkind = 0
        foreach($mk in $monkey){
            
            #$outstr = ("Monkey {0}: {1}" -f $mkind,($mk -join ", "))
            #Write-Verbose ($outstr)
            Write-host ("Monkey {0} inspected items {1} times" -f $mkind,$MkScore[$mkind])
            $mkind++
        }
    }
        ...
        if($true){
            $z = 0
            Write-host ("After {0} rounds" -f $rounds)
            foreach($score in $MkScore){
            Write-host ("Monkey {0} inspected items {1} times" -f $z,$MkScore[$z])
            $z++
        }
    }
