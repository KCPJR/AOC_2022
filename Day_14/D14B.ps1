Function D14B{
    ...
    Begin{
        ...
        foreach($line in $data){
            ...
            #now add line for floor
            $Halfwidth = $ymax * 2
            $floorstart = (500 - ($ymax + 1 + 1))#extra +1 is buffer
            $floorfinish = (500 + ($ymax + 1 + 1))#extra +1 is buffer
            Write-Verbose ("Floor: {0} to {1} at height of {2}" -f $floorstart, $floorfinish, ($ymax+2))
            $dx = ($floorstart)..$floorfinish 
            $dx|%{("{0},{1}" -f $_, ($ymax+2))}|%{$h[$_] = 'Floor'}
        ...
        while (!$gameover){    
            ...
            if( ($SandCounter + 1) -ge 90){
                Write-verbose ("Slow down!")
            }
            $stopped = $false
            $sand = "500,0"
            $x = [int]$sand.split(",")[0]
            ...
            while(!$stopped -and !$gameover){
                ...
                if($h.ContainsKey($newcoord)){
                    ...
                    $moved = $false
                    $stopped = $true
                    $lastplaced = ("{0},{1}" -f $x,$y)
                    if($lastplaced -eq "500,0"){
