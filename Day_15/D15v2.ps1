Function D15v2{
    ...
    Begin{
        ...
        for($i = 0;$i -lt $sensors.count;$i++){
            for($j = $i + 1; $j -lt $sensors.count; $j++){
                ...
                $sumofreach = $sensors[$i].dg + $sensors[$j].Dg
    ...
    $curloc = "0,0"
    #find sensor in range
    $SensorFound = $false
    $i = 0
    $triedallsensors = $false
    while ( !$triedallsensors){
        #Write-Host ("{0} : {1}" -f $curloc, $i)
        $disttosensor = Get-Distance -coordA $curloc -coordB $sensors[$i].coord
        $curx = [int]$curloc.split(",")[0]
        $cury = [int]$curloc.split(",")[1]
        $dx = $sensors[$i].x - $curx
        $dy = $sensors[$i].y - $cury
        
        if($disttosensor -le $Sensors[$i].d){
            ...
            $curx = [int]$sensors[$i].x + ($sensors[$i].d - [math]::abs($dy))  + [int]1
            if($curx -gt $max){
                $curx = 0;$cury++
            }
            if($cury -ge $max){$triedallsensors = $true}
            $curloc = ("{0},{1}" -f $curx, $cury)
            $i = 0
            
        }
        Else{
            $inrange = $false
            if($i -lt $sensors.count -1){
                
                $triedallsensors = $false
                $i++
            }
            else{
                $triedallsensors = $true
            }
    ...
    Write-host ("CurrentLoc: {0}" -f $curloc)    
    $solp2 = $curx * 4000000 + $cury
Write-host ("Solution Part 2: {0}" -f $solp2)
...
function Scrap{
    ...
    foreach($s in $sensorlist){
        $target = "14,11"
        $dtosensor = Get-Distance -coordA $target -coordB $s.coord
        [PSCustomObject]@{
            Sensor = $s.index
            Coordinate = $s.coord
            Dist2Target = $dtosensor
            Range = $s.D
            AdjustedRange = $s.Dg
            Relevant = ($distbetween -eq $s.dg)
        }
    }
